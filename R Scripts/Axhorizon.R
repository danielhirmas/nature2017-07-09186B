...
# for just the A horizons (aquic moisture regime samples removed)
# ---------------------------------------------------------------------------
...
if (rerun==T) {
  ...
  climatehold <- which(names(climate)=="Aresepx")
  ...
  da <- as.matrix(data.frame(lon=dAx$long,lat=dAx$lat))
  ...
  for (i in 1:nrow(stmat)) {
    ...
    if (length(hold)>0) {
      steff <- c(steff,mean(dAx$RESEPx[hold]))
    } else {
      steff <- c(steff,NA)
    }
  }
  climate <- data.frame(climate,Aresepx=steff)
}
# ---------------------------------------------------------------------------